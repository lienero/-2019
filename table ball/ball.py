...
class Ball:
    ...
    def move_next(self):
        ...
        # 공이 오른쪽 벽에 부딪쳤을 때:
        if(self.x_posn >= (self.table.width - (self.width - 1))):
            self.x_posn = (self.table.width - (self.width - 1))
            self.x_speed = -self.x_speed
        ...
        # 공이 아래쪽 벽에 부딪쳤을 때:
        if(self.y_posn >= (self.table.height - (self.height - 1))):
            self.y_pson = (self.table.height - (self.height - 1))
            self.y_speed = - self.y_speed
